...
# %%
def makeRztrace(vR0, vz0):
    xv_ini = np.array([8.21, 0.014, 0,
                   vR0/Phi.kpc_Myr_to_km_s,
                   vz0/Phi.kpc_Myr_to_km_s,
                   (-233.1-12.24)/Phi.kpc_Myr_to_km_s])

    t_eval=np.linspace(0,13000,2000)
...
def makeSoS(vR0, vz0):
    ...
    t_eval=np.linspace(0,13000,20000)
...
for i in range(10):
    R,z = makeRztrace(40,40+5*i)
    ax[i%5][i//5].plot(R,z)
    ax[i%5][i//5].set_aspect('equal')
    ax[i%5][i//5].set_title('vz = '+str(40+5*i))
#ax.axis('off')
plt.show()
...
#ax.imshow(img, extent=[-17, 17, -17, 17])
for i in range(10):
    R,vR = makeSoS(40,40+5*i)
    ax[i%5][i//5].plot(R,vR, 'x')
    ax[i%5][i//5].set_title('vz = '+str(70+i))
#ax.axis('off')
plt.show()
